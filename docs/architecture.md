...
**残っている穴: 応答の切り捨て (#68)。** `ai::complete` は `max_tokens` 固定で `stop_reason` を見ていないので、長い `index.ts` が途中で切れると JSON パース失敗としてしか観測できず、エンドユーザーには読めない文言が出る。仕様書 §5 が型の自己宣言を勧めている以上、生成物は長くなる方向にあるので実際に踏みうる。
