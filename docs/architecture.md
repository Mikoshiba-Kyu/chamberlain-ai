...
JS 実行 1 回に **110 秒**の予算をかける。止め方は 2 通り要る。**片方だけでは塞がらない。**

| 種類 | 例 | 止める仕組み |
|---|---|---|
| 非同期の待ち | 解決しない Promise、返ってこない `fetch` | rustyscript の `RuntimeOptions::timeout` |
| 同期のループ | `while (true) {}` | 番犬スレッド (V8 の `terminate_execution`) |

`RuntimeOptions::timeout` の実体は tokio の `timeout` で、JS が event loop に制御を返すことを前提にしている。同期の無限ループは制御を返さないので**タイマーが動く機会が無い** (実測: 60 秒待っても返らない)。逆に非同期の待ちは番犬では止められない — 待っている間 JS は動いていないので `terminate_execution` は次に JS が動くときまで効かない。実装は `crate::watchdog`。

中断しても Runtime は壊れない (実測)。作り直しは要らず、次のトリガーはそのまま動く。ただし `terminate_execution` の中断フラグは**トリガーごとではなく isolate ごと**なので、叩いたら必ず `cancel_terminate_execution` で降ろす。降ろし忘れると次に動かした別のトリガーが自分のせいでない中断を食う。
