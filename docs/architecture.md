...
### 起動時の突き合わせ (#26 決定事項 7 / 追加決定 9 / #50)

アプリのアップデートやアプリを閉じていた時間の経過で前提が変わるため、起動時に永続タスクリストを現在の manifest と突き合わせる。
...
- **長期停止からの復帰 (#50)** — 猶予を超えて古くなった schedule 由来タスクを一括で破棄し、**トリガーごとに 1 行**報告する。これは**再起動の経路にだけ効く**。プロセスを保ったままのスリープ復帰は心拍が 1 件ずつ扱う (#53)

最後のものは「心拍でもできること」を起動時に移した判断である。猶予を超えた schedule 由来タスクは絶対に実行されない (`classify_due` が `SkippedLate` を返す) が、48h 先まで展開された状態で 2 日閉じると `@every 5m` で 576 件になり、**同じことを言う `[skipped]` が観測面を埋めて起動時にしか出ない `[expanded]` / `[rescheduled]` を押し流す**。#42 で履歴が永続化されてからは retention も食う。判定は心拍と同一で、報告の粒度だけを変えている。

ad-hoc は対象外。猶予 24h 内なら遅れても実行しなければならず (決定事項 8)、超過分は手動由来で件数が知れているので心拍が `[expired]` として 1 件ずつ扱う。pause 状態を見ないのは **pause が永続化されていない**ため — 起動直後は必ず全トリガーが実行中である。
...
| `[paused]` | 停止中のため予定を破棄 |
| `[skipped]` | schedule 由来の予定が猶予を超えて遅れたため破棄 (心拍中に 1 件ずつ) |
| `[stale]` | 長期停止からの復帰で、猶予を超えた予定をトリガー単位に一括破棄 |
