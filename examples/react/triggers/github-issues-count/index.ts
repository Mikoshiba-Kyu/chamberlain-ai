// Type I トリガー (トリガー内でタスク AI を使う) のサンプル (#15)。
//
// 動作:
...
//
// スコープ外: 対象 repo の manifest 指定 (今はここで hardcode)、複数 repo、
// Draft / assigned などの詳細集計。

// 対象リポは Chamberlain 自身。実データで動くのが検証しやすい。
const REPO = "Mikoshiba-Kyu/chamberlain-ai";

// ---- framework 側から注入される globals の型宣言 ----
// core の bootstrap.js が globalThis.chamberlain を生やしている。
// 型は将来 core パッケージから import する形にしたい。
