...
- **トリガーの AI 消費に天井がついた (#82)。** 1 回の実行で `chamberlain.ai.complete` が使えるトークンの合計は既定 24576 で、使い切った後の呼び出しは**呼びに行く前に例外**になり `[denied]` が残る。`ai.complete` は 1 往復固定なのでエージェントループのような O(n²) は起きないが、**1 run あたりの呼び出し回数に上限が無かった** — `@every 5m` は 1 日 288 回動くので、1 run が数千トークン使うだけで月あたり億のオーダーに届く
  - **回数ではなくトークンで数える。** 10 件を要約するトリガーが 10 回呼ぶのは正常系なので、回数の上限は正常系を壊す。数えるのは `input` + `output` + `cache_creation` + `cache_read` の合計で、予算は実行ごとにリセットされる
  - 判定は**今回の `maxTokens` を足してから**行う。使い切ってから断る形だと予算は必ず 1 回分ぶん超過する。入力側は tokenizer を持たないぶん 1 回分だけ超えうる
  - 通した呼び出しは応答が返るまで `maxTokens` を**押さえる**。実消費だけで比べると `await` しない呼び出し (`Promise.all` で束ねたもの等) がどれも消費 0 の状態で判定され、予算が並行呼び出しで素通りする
  - 足りないトリガーは manifest に `maxTokensPerRun` (1〜4122880) を書く。**`maxRuntimeSec` と違って下げる方向にも書ける** — 下げると同意画面の見積もりが小さくなり承認されやすくなる。宣言が自分に不利にしか働かないなら誰も正直に書かない。範囲外は丸めずに構成エラー (#68 / #81 と同じ判断)
  - `permissions.rs` の「レート制限は必要になってから (#57)」がここに着地した
- **登録前に、そのトリガーが月あたり最大どれだけ AI を使うかが見えるようになった (#82 / #58)。** `schedule` と `maxTokensPerRun` は両方 manifest にあるので、登録する前に上限が計算できる。同意画面とトリガー一覧に「1 回の実行あたり最大 N トークン。月あたり最大 M 回動くので、合わせて最大 X トークン」が出る
  - **金額には換算しない** (#71 の線のまま)。モデル別の単価を core が抱えると #27 のカタログ保守を背負う。出すのはトークン数までで、単価はエンドユーザーの手元にある
  - 「秘書にコストを意識させる」は採らない。`ai.complete` に breakpoint の口が無い以上、指示された秘書が書けるコードは存在しない (#72)。コストは `allowedHosts` / `requiredSecrets` と同じカテゴリで、**秘書は宣言できても天井は framework が持つ**
  - `TriggerListItem` / `TriggerCandidate` に `maxTokensPerRun` と `runsPerMonth` が加わった。宣言していなくても既定の予算が入るが、**構成エラーのトリガーでは両方 null** — 「見積もりが出せない」を core が 1 回だけ言うことで、UI 側 (テンプレートの数だけある) がその判断を持たずに済む。掛け算は UI 側で行う (`u32` では溢れる組み合わせがある)
