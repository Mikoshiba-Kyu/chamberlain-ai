...
//! 避ける。wall-clock の missed-fire skip (#18) は「生成しない」ことで達成され、決定事項 8 の
//! 遅延分岐が効くのは「リストに積まれた後に時間が飛んだ」場合 (スリープ復帰・心拍の遅れ)
//! だけになる。
//!
//! # 猶予期間が origin で違う理由
//!
//! 猶予を 24h 一律にすると「10:00 に起動して当日 09:00 の挨拶が実行される」ことになり、
//! #26 決定事項 8 が挙げている「今更 09:00 の挨拶をしても不自然」と矛盾する。よって:
//!
...
//! # 将来の拡張について
//!
//! [`Task`] に `prompt` 等のフィールドは今は持たせない (トリガー実行のみ)。serde は未知
//! フィールドを既定で無視するので、Phase 3 でフィールドを増やしても古い `tasks.json` は
//! そのまま読める。逆方向 (新 → 旧) は 0.x の範囲で保証しない。
...
/// (決定事項 1)。過去の記録は [`crate::history`] の担当。
...
/// **その回にしか出ない `[expanded]` / `[rescheduled]` を押し流してしまう**。
///
/// 心拍だけから呼ぶ。プロセスを保ったままのスリープ復帰は起動時突き合わせを通らないので、
/// 掃除を心拍 1 箇所に置けば「起動か復帰か」の区別自体が要らなくなる (#53)。
///
/// # 対象
///
