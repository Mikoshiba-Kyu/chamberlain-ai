...
| `maxRuntimeSec` | | 1 回の実行に 110 秒では足りないときだけ書く (§6)。**書くと配送の約束が変わります** |
| `maxTokensPerRun` | | 1 回の実行で AI が使ってよいトークンの合計 (§6)。省略すると 24576。**多く AI を呼ぶトリガーは書いてください** |
...
- **1 回の実行で使えるトークンの合計にも上限があります** (既定 24576、入力と出力の合計)。使い切った後の呼び出しは例外になり `[denied]` が残ります。足りないトリガーは manifest に `maxTokensPerRun` を書きます (§6)。
...
- **既定では 1 回の実行は 110 秒まで**です。超えると中断され `[error]` が残ります。
- `setTimeout` で長時間眠るのは避けてください。「10 分後にもう一度見る」は、そういう `schedule` を書くか、state に記録して次回判断します。

#### 110 秒で足りないとき — `maxRuntimeSec`

フィードを集めて 30 本を要約して組み立てる、のように **AI 呼び出しを何度も積む仕事**は 110 秒に収まりません。その場合だけ manifest に秒数を書きます。
...
- 書ける範囲は **111〜1800 (秒)** です。範囲外を書くとそのトリガーは構成エラーになります (黙って丸めません)。
- **書かないのが既定です。** 110 秒で済む仕事に書かないでください。
- **時間が伸びてもトークンの上限は別です。** AI を何度も積む仕事は、次の節の `maxTokensPerRun` も見てください。

**書くと 2 つのことが同時に変わります。**

1. **走る場所が変わる** — 宣言したトリガーは専用の枠で実行されます。宣言していないトリガーの邪魔をしない代わりに、**宣言したトリガーどうしは順番待ち**になります。
2. **やり直されなくなる** — 通常のトリガーは、実行中にアプリが落ちると次の起動でもう一度実行されます。`maxRuntimeSec` を書いたトリガーは **実行されません**。AI 呼び出しを何十回も積む仕事を黙って焼き直すと、使う人の請求が倍になるためです。

長い仕事を書くときは、**途中経過を state に残す**ことを勧めます。1 回で終わらなくても次回に続きから進められ、落ちたときに失うものが減ります。

#### 実行の並びについて

- トリガーは**同時に複数動くことがあります**。同じトリガーが二重に動くことはありません (前回が終わるまで次は始まりません)。
- 他のトリガーとは state も鍵も通信先も共有されないので、同時に動いても干渉しません。
- **モジュールのトップレベル (`tick` の外) は複数回実行されることがあります。** 実行の枠ごとに 1 回ずつ読み込まれるためです。初期化のつもりで副作用を書かず、必要な処理は `tick` の中に置いてください。トップレベルに置いた変数は**実行のたびに引き継がれるとは限りません** — 持ち越したい値は必ず `state` に入れてください。

### AI の消費の制限

AI の呼び出しは **使う人の API キー、つまり使う人の請求**で動きます。時間の上限とは別に、**1 回の実行で使えるトークンの合計に上限があります** (入力と出力の合計、既定 **24576**)。

- 使い切った後の `ai.complete` は例外になります。呼ぶ前に断られるので、API には出ません。
- 上限は**回数ではなくトークン**です。10 件を短く要約するのに 10 回呼ぶのは正常な書き方で、そこを止めたいわけではありません。

足りない場合だけ manifest に書きます。
...
- 書ける範囲は **1〜4122880** です。範囲外を書くとそのトリガーは構成エラーになります (黙って丸めません)。
- **小さく書くこともできます。** AI をほとんど使わないトリガーが小さい値を書くと、登録画面に出る「月あたり最大」の見積もりがそのぶん小さくなり、使う人が承認しやすくなります。
- 見積もりは `schedule` × `maxTokensPerRun` で計算されて **登録前に使う人に見えます**。大きく書けば通るというものではありません。

### 頻度は必要最小限に

`schedule` は**コストを決める一番大きな要素**です。`@every 5m` は 1 日 288 回、1 か月で約 8900 回動きます。1 回の実行が数千トークン使うなら、それだけで月あたり数千万トークンになります。

- **「早く気づきたい」と「頻繁に見る」は別です。** 朝に 1 回まとめて知らせれば足りる仕事に `@every 5m` を書かないでください。
- 外部の更新を追う仕事でも、更新そのものが日に数回なら `@hourly` で足ります。
- 迷ったら**粗い方**から始めてください。細かくするのは後からできます。
...
- [ ] その `schedule` の頻度が**本当に必要な最小限**になっている (§6)
...
- [ ] 1 回の実行で AI を何度も呼ぶなら `maxTokensPerRun` を宣言してある (§6)
